...
box allows incoming connections to the external (``public'') IPs
...
interface of the gateway should be designed on their needs.
...
\item
  Each resitental home needs a WLAN controller (costly)
\item
...
The technical issue will be investigated in the future. The reason it may not
work in our environment due to the determination of the home agent in the network.
According to the documentation the calculation is based on the user VLAN IP subnet \cite[p. 661]{MobileIP-Aruba}.
To work properly, VLAN ids may not be unique in the whole network.
...
altering and analysing pretty flexible. Figure \ref{Hooks.pdf} shows an overview of the different hooks an how traffic flows
...
(for example 2 different networks you want to use NAT for). Destination NAT
is just the opposite and requires to be performed before the routing,
otherwise its not clear what the actual destination is and how it has to be routed.
