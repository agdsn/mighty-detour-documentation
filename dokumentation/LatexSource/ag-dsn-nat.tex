...
\titlepic{\includegraphics[width=0.70\textwidth]{../agdsn_logo_quadratisch.pdf}}
...
each student. Furthermore, IPv4 addresses are non-optional since not every service provider in the internet
supplies IPv6 connectivity right now. The fact that everybody owns more than one IP address capable device
...
information of a network packet \cite{NAT}. It is often required, because there are
not enough public IPv4 addresses for all hosts of a network \cite{IPv4Ex}. A private
...
NAT mode in use by consumer routers \cite[SNAT - Source Network Address Translation]{SDNAT}.

\paragraph{Destination NAT (DNAT)}

Destination NAT is required if a service located on an internal host should be available on
the public network \cite[DNAT - Destination Network Address Translation]{SDNAT}. When the gateway receives traffic from the public
...
the usage of the 32 bit IPv4 address space \cite{cidr}. It is used in this document
to describe subnets and their size.

\section{Features}\label{features}

This section describes the different goals we're trying to achieve by the project.

\subsection{Private Networks}\label{private-networks}

Every user of the network gets a private network (also referred to as ``home network''). On layer 2
this network is realized by using a VLAN according to IEEE 802.1Q \cite{802.1Q} for every user. This
network will be located at the router next to the users flat. If the user tries to connect to the
network via LAN or WLAN inside his building, he will be placed inside this network. Since the number
of VLANs is limited (4096), the VLANs are only unique in a dormitory. %This implies that all
% entirety networks are only distinct
% inside the building,
%not in a global (whole of residential homes) context.
The IPs assigned inside a private network are going to be RFC6598 addresses \cite{CGN}, in practice
an /24 network out of the Carrier-Grade NAT (CGN) range (100.64.0.0/10). Those layer 3 networks will
be unique in the AG DSN network. Every layer 3 network will be mapped via network address
translation (NAT\cite{NAT}) to an external IP address on the NAT gateway. Every external IP address
is only in use by a single user.


\subsection{Roaming Networks}\label{roaming-networks}

All buildings get a unique layer 2 network (VLAN) for roaming purposes. When a user connects to the network
...
\subsection{Bandwidth Throttling}\label{bandwidth-throttling}
...
bandwidth throttling \cite{throttl} will be performed by the NAT gateway. It is possible to 
configure exceptions, for certain destinations, that are excluded from the throttling.
...
\paragraph{Active - Active (Symmetric Routing)}
...
connection get translated by the same gateway (Symmetric Routing).

\paragraph{Active - Active (Asymmetric Routing)}
...
\subsubsection{Advantages}\label{pro}
...
\subsubsection{Possible usage at AG DSN}

If a user tries to connect to the network in a different residential home apart from his rented flat, his private network (see \ref{private-networks}) may not be available. IP mobility can obsolete roaming networks (see \ref{roaming-networks}) by placing roaming users in their actual private network.
...
otherwise its not clear what the actual destination IP address is and where the packet has to be routed.
...
If matched data is inside the table, the application rewrites
...
utilitise multi core processors \cite{pfOpenBSD}. This may cause performance issues,
...
This allows us to use asymmetric routing with our gateways, meaning that a
...
To ensure a good performance of our system we made multiple optimizations to the default debian
configuration, the used scripts and commands can be found in the appendix:

\paragraph{Conntrack Table:} The conntrack table size and bucket number determine how many
connections can be active at the same time and how long a single look up takes. We raised the
conntrack table size to $2^{24}$ and the bucket number to $2^{22}$ (average of $2^2$ conntracks per
bucket). With a conntrack size of about 350 bytes, 5,9 GB of RAM are needed to hold a full conntrack
table. Another optimization for the conntrack table is the timeout reduction for established tcp
connections. We reduced the timeout from 432000 seconds (5 days) to 3600 seconds (1 hour), this
should reduce the number of connections in the conntrack table.

\paragraph{Interrupt handling:} To allow the kernel to handle more packets in a single interrupt we
increased the netdev\_budget and NIC receive queue size to 4096. This should reduce the number of
interrupts and therefore the needed overhead for context switches in the CPU.

\paragraph{Flow control:} Packets are usually destined for a specific application. Modern NICs match
traffic to the CPU core that is executing the destined application using flow control. This allows
efficient processing of network traffic on multi-core CPUs. Since the network address translation is
performed by the kernel, any core can be used. This is why we disabled flow control and used static
CPU affinities for the receive and transmit queues, removing the overhead of flow control.

\section{Kernel Modules}



In figure\ref{KernelModules.pdf} the required kernel modules for NAT and their dependencies are
shown. The modules with "nf" as prefix are part of the netfilter library, the ones with "nft" are
new kernel modules from nftables. The dependency tree shows the connection between the NAT modules
and the conntrack module.

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.75\textwidth]{../KernelModules.pdf}
	\label{KernelModules.pdf}
	\caption{The required kernel modules and their dependencies}  
\end{figure}
...
Fortunately, we were able to work around these two problems.
The first issue is addressed by simply limiting the number of connections a client address block can open.
Sadly, nftables is not capable of performing such an operation (yet). Using the iptables-compat toolset - which is intended
to simplify the migration from iptables to nftables tooling - we are able to setup an limiting rule.
This is possible since the iptables-nftables-compat use the nf\_tables kernel subsystem \cite{iptables_compat}.
The following statement resolves the problem:
\begin{verbatim}
iptables-compat -t filter -A FORWARD -m conntrack --ctstate NEW -m connlimit --connlimit-above 100 --connlimit-saddr -j DROP
\end{verbatim}
Obviously this is a rule appliced to the filter-table. It sets the connection limit for each source group
(for each private network).

Our secondary problem was resolved by just updating the firmware of the NIC. We were not able to determinate the exact
cause for the misbehavior, since the changelog of the manufacturer does not contain any related information.


\chapter*{Acknowledgement}

We would like to thank Richard Moerbitz and Sebastian Schrader for their great support during this project.
...
\begin{verbatim}
#!/bin/bash
# Allow more packets to be processed during interrupt handling
sysctl -w net.core.netdev_budget=4096
# Increase conntrack hash table
sysctl -w net.netfilter.nf_conntrack_buckets=4194304
sysctl -w net.netfilter.nf_conntrack_max=16777216
# Reduce conntrack timeouts
sysctl -w net.netfilter.nf_conntrack_tcp_timeout_established=3600
# Disable Ethernet flow control for net7
ethtool -A net7 tx off rx off autoneg off
# Increase queue size for net7
ethtool -G net7 rx 4096 tx 2048
# See IRQ affinity 
/root/set_cpu_affinity.sh net7 {0..26..2}
...
pushd /sys/class/net/"$device"/device/msi_irqs >/dev/null
declare -ar irqs=(*)
popd >/dev/null

echo "Setting IRQ affinity and XPS maps for $device"

declare -i i j
for (( i=0; i < ${#irqs[@]}; i++ )); do
    cpu_id="${cpus[$(( i % ${#cpus[@]} ))]}"
    cpu_mask="$(printf '%x' $(( 1 << (${cpu_id} % 32) )))"
    for (( j=0; j < ${cpu_id} / 32; j++ )); do
        cpu_mask="${cpu_mask},00000000"
    done
    irq="${irqs[$i]}"
    echo "$cpu_id" > /proc/irq/"$irq"/smp_affinity_list
    echo "$cpu_mask" > /sys/class/net/"$device"/queues/rx-"$i"/rps_cpus
    echo "$cpu_mask" > /sys/class/net/"$device"/queues/tx-"$i"/xps_cpus
done
\end{verbatim}
\endgroup
