...
def rule_exists(value, table, chain=None):
    ...
    if chain is None:
        ...
        command = cfg()['netfilter']['nft']['call'] + " list table " + table + " -a -nnn"
    else:
        command = cfg()['netfilter']['nft']['call'] + " list chain " + table + " " + chain + " -a -nnn"
    logging.debug("Execute: " + command)
    output = subprocess.check_output(command, shell=True).decode("utf-8").replace("\\t", "").replace("\\n", "").splitlines()
    matched_output = []
    for l in output:
        if str(value) in l:
