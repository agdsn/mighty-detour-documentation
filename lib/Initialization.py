...
def initialize(translations, throttles, forwardings, blacklist, whitelist):
    ...
    src += "    map " + cfg()['netfilter']['throttle']['map'] + "_cgn {\n"
    src += "        type ipv4_addr: verdict;\n"
    src += "        flags interval;\n"
    src += "        elements = {\n"
    src += "            " + ','.join(generate_throttle_map_elements_cgn(throttles)) + "\n"
    src += "        }\n"
    src += "    }\n"
    src += "\n"
    src += "    map " + cfg()['netfilter']['throttle']['map'] + "_inet {\n"
    src += "        type ipv4_addr: verdict;\n"
    src += "        flags interval;\n"
    src += "        elements = {\n"
    src += "            " + ','.join(generate_throttle_map_elements_inet(throttles)) + "\n"
    src += "        }\n"
    src += "    }\n"
    src += "\n"
    ...
    src += "        ip daddr vmap @ " + cfg()['netfilter']['throttle']['map'] +"_cgn\n"
    src += "    }\n"
    src += "\n"
    src += "    chain ratelimit_map_inet {\n"
    src += "        ip saddr vmap @ " + cfg()['netfilter']['throttle']['map'] +"_inet\n"
    src += "    }\n"
    src += "\n"
