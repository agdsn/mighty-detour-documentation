...
global public_ip
public_ip = ipaddress.ip_address("141.30.202.0")


def printMapList():
	net = ipaddress.ip_network("100.64.0.0/10", False)

	public_ip = ipaddress.ip_address("141.30.202.0")

	subnets = net.subnets(prefixlen_diff=14)
	i = 0
	for sub in subnets:
		public_ip += 1
		#print(sub)
		#print("nft add element nat subnettoip5 { " + str(sub) + ":" + str(public_ip) + "}")
		print( str(sub) + ": " + str(public_ip) + ", ", end="")
...
def createLeafs(net, pref):
	global public_ip
	subnets = net.subnets(prefixlen_diff=3)
	for sub in subnets:
		print(nftBase + "add rule " + table + " " + pref + " ip saddr " + str(sub) + " oif " + interface + " snat " + str(public_ip))
		public_ip += 1
...
def generateTree():
	...
	print(nftBase + "add rule "+table+" postrouting ip saddr 100.64.0.0/12 oif "+interface+" goto postrouting-level-0")
...
def generateRateLimitMap():
	...
	for sub in subnets:
		...
		print(nftBase + "add element filter iptoverdict { " + str(sub) + " : goto " + " ratelimit-" + str(sub) + " }")
